...
- **Exam manifest execution surface.** An `exam.yaml` declares `validations` that run on the control plane (e.g. `kubectl`). Reports of ways a malicious manifest could escape its expected sandbox are highest priority.
